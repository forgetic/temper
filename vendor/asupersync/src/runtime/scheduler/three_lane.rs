...
impl ThreeLaneWorker {
    ...
    fn drive_io_phase(&self) -> IoPhaseOutcome {
        ...
        let io_timeout = if self.fast_queue.is_empty() {
            // PATCH(temper): bound the leader's blocking poll. With no known
            // deadline the leader used to enter the reactor with no timeout;
            // a timer registered after that point never kicks the reactor,
            // and followers deliberately ignore shared timer deadlines, so a
            // bare `sleep` on an otherwise idle runtime never fired (lost
            // wakeup, observed empirically). Re-polling at least every 25ms
            // lets the next_task() timer pump fire due timers.
            const MAX_IO_PARK: Duration = Duration::from_millis(25);
            Some(timeout.map_or(MAX_IO_PARK, |window| window.min(MAX_IO_PARK)))
        } else {
            Some(Duration::ZERO)
        };
