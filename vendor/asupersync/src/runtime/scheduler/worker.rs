...
impl Worker {
    ...
    pub fn run_loop(&mut self) {
        ...
        while !self.shutdown.load(Ordering::Relaxed) {
            // PATCH(temper): fire due wall-clock timers. Upstream 0.3.1 only
            // pumps the timer wheel from the lab runtime; in the production
            // scheduler, sleeps/timeouts registered with the ambient timer
            // driver would otherwise only complete when unrelated activity
            // happens to re-poll their tasks. The 25ms park below bounds the
            // added latency for an otherwise idle worker.
