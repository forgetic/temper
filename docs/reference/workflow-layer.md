...
Queue automation metadata now lets a queue declare mechanical servicing:
`automation.actor` is a declared role id, `automation.transition` is the primary transition to execute for matched active members, and `automation.on_merge_conflict` is an optional same-actor fallback transition for merge conflicts. Validation checks those references, role authority, and artifact-kind compatibility. The compiled `QueueManifest` carries this metadata so runners can discover automated queues without reparsing raw specs; Phase 1 only exposes the contract and does not execute automated queues.
