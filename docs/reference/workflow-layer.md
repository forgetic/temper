...
The reconciler scans Forge artifacts and the command journal and decides what to repair or escalate; `recover::Applier` then applies the decision through the executor, lease manager, and journal (see "Applying reconciler actions"). `Reconciler::scan` is pure and deterministic: given `ArtifactSnapshot`s, `CommandRecord`s, a `DependencyStatus`, and `now`, it returns a `ReconcileReport` whose parallel `findings` and `actions` follow a stable order. Snapshot loading is explicit: bounded `Reconciler::reconcile` loads exact snapshots for incomplete journal targets, discovers workflow-labelled issue/PR candidates with explicit state+label summary queries, deduplicates them, and reloads exact detail for dependency-gated artifact kinds before deriving `DependencyStatus`. `reconcile_bounded` remains the exact-journal plus caller-supplied candidate entry point. Missing exact targets are simply absent so the existing stale-command finding is produced. Runner `MechanicalWorker::tick` uses bounded reconciliation; `tick_deep_audit` and production/testing `--audit-ms` ticks are the explicit all-history operator path. `Reconciler::reconcile_deep_audit` loads all visible issues and pull requests with full detail before deriving `DependencyStatus`. Child-repo read failures do not abort the whole scan; the unreadable target simply remains not landed so `Unblock` is not produced prematurely.
