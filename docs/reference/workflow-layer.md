...
`automation.actor` is a declared role id, `automation.transition` is the primary transition to execute for matched active members, and `automation.on_merge_conflict` is an optional same-actor fallback transition for merge conflicts. Validation checks those references, role authority, and artifact-kind compatibility. The compiled `QueueManifest` carries this metadata so runners can discover automated queues without reparsing raw specs. A normal mechanical worker tick now runs bounded reconciliation first, then scans only automated queues through the bounded queue-candidate path and executes each item through `Executor::execute` with the declared actor and transition. Expected stale preconditions and gate misses are logged as unchanged so later polls or wake hints can retry after Forge state changes; a typed `MergeConflict` runs the queue's `on_merge_conflict` fallback when configured, again through `Executor::execute`; backend, validation, classification, and impossible-state failures remain visible errors.
