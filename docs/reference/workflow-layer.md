...
Relation resolution: native artifact dependency links are the source of truth for same-repository `dependency` relations. If an artifact has no native dependencies, metadata `dependencies` is used as a compatibility fallback and may carry repo-qualified `ArtifactRef` values. Metadata `parents` still feeds `parent` relations, and `produced_pr` declarations are read from the `parents` projection on PR artifacts. The classifier emits `ClassifiedRelation`s for declarations whose source is the artifact kind, preserving the linked `ArtifactRef` and declared possible target artifact kinds.
