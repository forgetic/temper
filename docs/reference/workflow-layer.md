...
- `QueueManifest` adds `subscribers`, multi-kind/disjunctive matching, activation policy, and optional automation metadata to each queue; `TransitionManifest` is the runtime transition table; `LabelManifest`/`LabelSpec`/`LabelUsage` enumerate every label a workflow site needs and why.
...
- `Executor::read_gate_signals` exposes the read-only load/classify/signal path for runner scans that need the full `GateSignals` bundle, and `read_gate_signals_with_needs` lets scans load only dependency, CI, or review families that a candidate queue can inspect.
- `ExecutionError` separates the failure classes a runtime must distinguish: `Validation` (undeclared transition, unauthorized role, artifact-kind mismatch), `Precondition` (stale/contradicted labels, unsatisfied gate, impossible state), `Backend` (any non-routable `ForgeError`), plus `Classification`, `TargetMissing`, `TargetStale`, `MergeConflict`, `UnsupportedEffect`, `UnresolvedAssignee`, `MissingCorrelationKey`, `UnresolvedPullRequestCreate`, and `PostconditionFailed`.
- `Executor::ensure_issue` and `Executor::ensure_pull_request` are idempotent create helpers: they search explicit artifact states with summary, body-marker, and create-label list queries; parse metadata to confirm the exact `correlation_key`; return the existing artifact if found; or stamp the key into the new metadata block and create it. `EnsureOutcome` reports whether the artifact was `Created` or `Existing`.
...
- `Executor::execute` applies `MergePullRequest` through the `Forge` merge API. The merge runs *before* the label/assignee commit point and is skipped when the freshly loaded pull request is already merged, so a merge is applied at most once even when a crash lands the merge but loses the response. A merge `ForgeError::Conflict` is re-read before classification: already merged continues to post-merge projection, missing/closed becomes stale/target failure, and still-open unmerged becomes `ExecutionError::MergeConflict`.
...
- `Executor::ensure_pull_request` mirrors `ensure_issue`: it searches explicit PR states with summary, body-marker, and create-label list queries; confirms the exact metadata `correlation_key`; stamps that key into new PR bodies; and returns `EnsureOutcome::Existing` on retry.
...
Queue automation metadata lets a queue declare mechanical servicing:
`automation.actor` is a declared role id, `automation.transition` is the primary transition to execute for matched active members, and `automation.on_merge_conflict` is an optional same-actor fallback transition for merge conflicts. Validation checks those references, role authority, primary artifact-kind compatibility with the queue, and fallback artifact-kind compatibility with the primary transition. The compiled `QueueManifest` carries this metadata so runners can discover automated queues without reparsing raw specs. A normal mechanical worker tick runs bounded reconciliation/apply first, then scans only automated queues through the bounded queue-candidate path. Automated work is emitted in queue declaration order and artifact-number order, then executed serially within the repository through `Executor::execute` with the declared actor and transition; there is no FIFO or head-of-line block across unrelated PRs. Expected stale preconditions and gate misses are logged as unchanged so later polls or wake hints can retry after Forge state changes; a typed `MergeConflict` runs the queue's `on_merge_conflict` fallback when configured, again through `Executor::execute`; backend, validation, classification, and impossible-state failures remain visible errors.
...
| `queue` | Query that selects artifacts needing attention by artifact kind(s), label filters, optional native/projected condition, optional read-side activation policy, and optional mechanical servicing metadata |
...
Labels are a portable Forge projection of workflow-owned state; native CI, dependency links, and review decisions are observed from the Forge instead of mirrored as labels. `ci_passed` and `ci_failed` are computed from fresh `CiJob` status/conclusion data, scoped to the pull request's current head SHA when the backend supplies one, not from `testing-*` labels. Review gates use the requested-reviewer aggregate and are not head-SHA-scoped in the portable model. A `relation` declares `{ kind, source, target }`, where `kind` is `parent`, `dependency`, or `produced_pr` and endpoints are artifact kinds. Non-label effect payloads stay portable: assignee and reviewer-request effects reference declared role ids (the runtime resolves a role to a concrete worker/user), comments carry a prose/template `body`, `create_pull_request` carries only an optional `correlation_key` while branch, title, body, labels, assignees, and (when the spec leaves it empty) a runtime correlation key come from runtime context, `submit_review` carries a portable decision, and `merge_pull_request` has no payload. The workflow layer uses native Forge dependency links for `dependency`; metadata blocks still carry correlation keys plus `parent`/`produced_pr` links and fallback dependency numbers.

The `reference-delivery.json` fixture transcribes the reference delivery design (`docs/explanation/reference-workflow.md`) into these primitives; its orthogonal lifecycle labels plus artifact-scoped state legality, non-label effects, native CI/review gates and queues, relation declarations, the relation-driven `dependency_gate`, queue activation/matching policy, and automated mechanical landing queue validate, compile, and plan (`tests/reference_delivery.rs`). Any remaining gaps are tracked in `docs/explanation/reference-workflow-gaps.md`.
...
Generated prompt prose is mechanical and role-id agnostic. The compiler renders workflow/role identity, concurrency, the work-item context contract, subscribed queues, authorized workflow actions, declared external-tool availability rules, and the authority boundary that executable workflow mutations come only from the compiled workflow tool manifest. Role-specific judgment such as engineering standards, review criteria, or owner values belongs in the user prompt fields or fixtures, not in compiler code or production Temper code. A role named by queue automation is authority for that automated transition only; the automation block does not subscribe the role to the queue, bind external tools, or create an LLM/process worker. Production role workers call configured process responders; any fixed reference-delivery prompts are test/demo fixtures outside the production role-worker path.
...
- queue automation blocks whose actor, transition, or conflict fallback references are missing, whose transition/fallback does not authorize the actor, or whose artifact kind is incompatible with the queue (implemented)
- unknown workflow, prompt-extension, external-tool, or queue-automation fields at load time through serde `deny_unknown_fields` (implemented)
...
Queue matching: a classified artifact matches a queue when its kind is one of the queue's artifact kinds, every common `labels` entry is present, either `any_of` is empty or at least one `any_of` clause's labels are all present, and any queue `condition` is satisfied by classified state or runtime signals such as `ci_failed` or `review_changes_requested`. The matcher exposes the kind/label stage separately from the condition stage so runtime scans can first discard non-candidates, then fetch only the dependency, CI, or review signals required by the cheap-matched queues. Label and state conditions need no runtime signal reads. The raw schema keeps the legacy `artifact: "code"` shorthand and also accepts `artifact: ["code", "implementation_pr"]`; `any_of` is an array of label-set objects. Because exclusive state dimensions project to mutually exclusive labels, a `code + ready` queue naturally excludes `blocked` and `in-progress` code issues. Matching does not consider activation policy or automation metadata.
...
Runner scans are staged by queue interest, not by repository history. A normal role scan considers only queues subscribed by that role, plans Forge list queries from those queues' artifact targets and label sets, lists open candidates plus closed/merged candidates only when a non-empty queue label can select them, classifies the summary records, and then reads only the signal families needed by cheap-matched queues. Therefore unlabelled closed artifacts are not part of ordinary ticks, and a role with no CI-gated candidate queue performs no CI reads. Audit scans broaden queue interest to all workflow queues and active/recoverable workflow-label recovery interest, but they still do not issue closed-all queue queries or terminal pure artifact-kind identity queries; exact dependency targets are loaded by reference when dependency gates need them. Reconciliation candidate discovery is similarly workflow-labelled: normal reconciliation queries each declared workflow label for open issue/PR states with summary list detail, but terminal closed issues and closed/merged PRs are queried only by target-relevant queue labels, state labels, transition effect labels, and label/state gate conditions. Pure artifact-kind identity labels such as a completed implementation PR's `implementation` label are not terminal broad-discovery labels. Bounded reconciliation never uses default all-state lists, and reloads exact artifacts only for incomplete journal targets or when a candidate's kind has dependency-gated recovery transitions.
...
3. compute the runtime gate signals required by the target transition, such as native dependencies, native CI, and native reviews
...
`Executor::execute` implements this loop today for labels, assignees, comments, pull-request creates, reviewer requests, review submissions, and pull-request merges. Queue automation metadata does not bypass this loop: automated queue servicing executes the declared transition through the same fresh-read, gate-checking executor path after a read-only bounded scan. The executor never trusts a plan computed against stale state: it re-loads, derives the transition's `SignalNeeds`, computes only those gate signals (dependency status by freshly reading every dependency target in its own repository, native CI from `list_ci_jobs`, and native review status from requested reviewers plus `list_pull_request_reviews` for PRs), and re-plans against fresh state immediately before mutating. A dependency target whose repository is temporarily unreadable remains not landed for that scan. When a reference backend has an issue and pull request with the same number, dependency resolution treats the issue as authoritative and only falls back to the pull request if the issue is absent. `Executor::read_gate_signals` exposes the full read-only load/classify/signal portion for runners that need all signals; `Executor::read_gate_signals_with_needs` is the lazy variant used by scans after cheap queue matching, and `Executor::read_classified_gate_signals_with_needs` also returns the freshly classified artifact so dependency-gated queues can evaluate against exact links after summary listing. The executor refuses to mutate at all if planning fails, if a plan contains an unsupported effect, if an assignee/reviewer role has no runtime user binding, if a create lacks both a spec and runtime correlation key, or if no pull-request create input is bound. It posts idempotent comments first, ensures pull requests next, requests reviewers and submits idempotently marked reviews, merges the target pull request next (skipping an already-merged target), then applies label and assignee changes together in one backend update. Because creates and merges precede the label commit point, retries can dedupe a landed create and finish post-create state, while post-merge labels become the marker that makes a retry refuse to re-run; already-merged targets are skipped, so the merge is at most once. If the merge API returns `ForgeError::Conflict`, the executor re-reads the pull request before returning: merged means the response was stale and projection continues, missing/closed is `TargetMissing`/`TargetStale`, and open/unmerged is `MergeConflict`. Postconditions are verified against the artifact state returned by the commit update, so a concurrently running worker cannot make a successful transition look failed by advancing the artifact before a later reload; a mismatch yields `PostconditionFailed`.

Idempotency: re-running a label transition that already applied fails as a `Precondition` error (the source label is gone and/or the target label is present), so a retry never double-applies. `SetAssignee` is cleanly idempotent when the resolved user is already assigned, and `RemoveAssignee` is cleanly idempotent when the resolved user is already absent. `CreateComment` is guarded by a hidden marker appended to the body (`<!-- temper:comment-key=<transition>:<comment-index> -->`); the executor lists comments on the same target and skips posting when the marker already exists, so a retry after a crash-before-state-flip cannot duplicate the comment. Comments have no postcondition; instead the marker check is the verified idempotency mechanism. Idempotent artifact create is handled by `Executor::ensure_issue`, `Executor::ensure_issue_with_parent`, and `Executor::ensure_pull_request` through correlation keys. Normal create lookup is bounded: issues query open and closed states, pull requests query open, closed, and merged states, list detail is summary-only, create labels are included when available, and a body substring marker is only a narrowing hint before exact metadata parsing. The proof does not depend on provider-side body search: a backend may apply `body_contains` client-side after state/label narrowing, but the executor still parses the workflow metadata block and compares the exact key. The parent-aware issue helper also repairs a found issue that has the key but lacks the required repo-qualified parent back-reference.
...
The reconciler scans Forge artifacts and the command journal and decides what to repair or escalate; `recover::Applier` then applies the decision through the executor, lease manager, and journal (see "Applying reconciler actions"). `Reconciler::scan` is pure and deterministic: given `ArtifactSnapshot`s, `CommandRecord`s, a `DependencyStatus`, and `now`, it returns a `ReconcileReport` whose parallel `findings` and `actions` follow a stable order. Snapshot loading is explicit: bounded `Reconciler::reconcile` loads exact snapshots for incomplete journal targets, discovers workflow-labelled issue/PR candidates with explicit state+single-label summary queries, deduplicates them, and reloads exact detail for dependency-gated artifact kinds before deriving `DependencyStatus`. Open candidates use every workflow label for the target type. Terminal closed issues and closed/merged PRs use only active or recoverable labels for that target: queue labels, state labels, transition effect labels, and label/state gate conditions. Pure artifact-kind identity labels alone are terminal-inert, so merged PRs labelled only `implementation` are left to exact journal recovery or deep audit instead of every bounded tick. `reconcile_bounded` remains the exact-journal plus caller-supplied candidate entry point. Missing exact targets are simply absent so the existing stale-command finding is produced. Runner `MechanicalWorker::tick` uses bounded reconciliation; `tick_deep_audit` and production/testing `--audit-ms` ticks are the explicit all-history operator path. `Reconciler::reconcile_deep_audit` deliberately loads all visible issues and pull requests with default full detail before deriving `DependencyStatus`; callers must use this only for operator diagnostics, not normal ticks. Child-repo read failures do not abort the whole scan; the unreadable target simply remains not landed so `Unblock` is not produced prematurely.
...
- `QueueManifest` entries with subscribers, multi-kind/disjunctive filters, activation policy, and optional automation metadata, for runtime queue evaluation
