...
Runner scans are staged by queue interest, not by repository history. A normal role scan considers only queues subscribed by that role, plans Forge list queries from those queues' artifact targets and label sets, lists open candidates plus closed/merged candidates only when a non-empty queue label can select them, classifies the summary records, and then reads only the signal families needed by cheap-matched queues. Therefore unlabelled closed artifacts are not part of ordinary ticks, and a role with no CI-gated candidate queue performs no CI reads. Audit scans broaden queue interest to all workflow queues and workflow-label recovery interest, but they still do not issue closed-all queue queries; exact dependency targets are loaded by reference when dependency gates need them. Reconciliation candidate discovery is similarly workflow-labelled: normal reconciliation queries each declared workflow label across explicit issue/PR states with summary list detail, never uses default all-state lists, and reloads exact artifacts only when their kind has dependency-gated recovery transitions.
