...
| --- | --- | --- | --- | --- | --- |
| CLI | `temper_cli::USAGE`, `temper_cli::dispatch`, `temper_cli_daemon::parse_serve_invocation` | root `README.md`; [operator compatibility surfaces](operator-compatibility-surfaces.md) | `operator_help_contract::{top_level_help_exposes_only_the_public_operator_contract,hidden_compatibility_and_internal_commands_remain_dispatchable}`; `target_ux::compatibility` | `init`, `check`, `plan`, `apply`, and `serve` are public; `daemon`, `agent`, and `trigger-forgejo` are hidden compatibility/internal dispatch | **Converged** |
