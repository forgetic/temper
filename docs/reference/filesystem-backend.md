...
`list_pull_requests` supports `PullRequestQuery` filters for state, conjunctive labels, author ID, assignee ID, and `details.dependencies`. Without a requested sort, pull requests are sorted by number ascending, then pull-request ID. `ItemSort` supports number, creation time, and update time with the requested direction; ties use number ascending, then pull-request ID. `details.dependencies=false` returns matching pull requests with empty dependency vectors without changing stored links.
