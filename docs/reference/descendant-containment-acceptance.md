...
out-of-band emergency KILL, emits bounded crash handoff diagnostics, and exits
with distinct status 70 through a core-dump-free primitive. It does not unwind,
run C/Rust exit handlers or owner drops, flush userspace buffers, or fabricate
`AttemptQuiesced`, cleanup, trace acknowledgement, result publication, or normal
assignment release. Startup staging and orphan/feed convergence recover the
retained assignment; exact attempt fencing rejects late old-process effects.
