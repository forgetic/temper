...
- `--poll-ms <n>` controls the normal broad poll cadence. Poll ticks scan the
  configured repository set using normal queue-derived candidate queries.
- `--audit-ms <n>` controls the low-frequency audit cadence. The default is
  ...
  unlabelled closed history. Mechanical audit ticks run the broad reconciler path.
...
A only for role workers) and to spot unexpected broad scans. When
