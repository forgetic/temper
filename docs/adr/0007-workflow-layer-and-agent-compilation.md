...
A workflow specification should combine structured machine-readable rules with user-authored role prompt guidance. Structured rules define artifacts, labels, state dimensions, queues, transitions, gates, invariants, and recovery policy. Legacy prose charters and structured prompt guidance guide judgment-heavy agent behavior such as design taste, review standards, and owner values; they are not generated by the compiler and grant no authority by themselves.
...
Agents should not receive broad Forge tools by default. The workflow compiler should generate the smallest useful tool surface for each role, with each tool enforcing workflow preconditions and applying only authorized transitions. Generated prompts should explain workflow mechanics, context, queues, actions, and authority boundaries, while role-specific behavior remains in user-authored prompt guidance.
