...
- `harness-workflow` is the workflow/orchestration crate; it provides the typed workflow spec (`RawWorkflowSpec`), typed ids, validation diagnostics, and `ValidatedWorkflow` (Phase 2), plus artifact/Forge target mapping, workflow metadata blocks, and a classifier for Forge issues and pull requests (Phase 3), plus compilation of a validated workflow into role/prompt/tool/queue/label manifests and a runtime transition table (Phase 4), plus a pure planner (`plan::Planner`) for queue matching and transition planning into typed `WorkflowEffect`s and postconditions (Phase 5). It depends on `harness-forge`. Runtime execution and recovery are still planned (see ADR 0007).
...
- The next likely work is continuing `harness-workflow` in phases (Phase 6: execute planned transitions through `harness-forge`), or adding another concrete Forge backend.
