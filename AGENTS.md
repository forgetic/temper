...
- `harness-runner` contains backend-agnostic reusable runner primitives and currently provides read-only `scan`/`scan_role` functions that turn fresh Forge state into active-queue `WorkItem`s for subscriber roles.
