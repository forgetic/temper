...
- `harness-forge-forgejo` is the first real-world provider backend (the `harness-forge-<provider>` convention), adapting the `Forge` interface to Forgejo's HTTP API through a mockable `HttpClient` seam; it is offline-tested only (no live tests). Its pull-request surface is implemented: list/get/create/update pull requests, PR comments (Forgejo issue comments), requested-reviewer requests, native review events with one-call submission, and merge, with label/assignee updates reusing shared issue-endpoint helpers (`items.rs`) and best-effort optimistic concurrency via a per-artifact `ETag`/`updated_at` validator cache (`VersionCache`, `CasMode`). Current user, repositories, repository labels, full issue CRUD, dependency links, CI, and the `Forge` trait wiring land in later phases. See `docs/reference/forgejo-backend.md`.
