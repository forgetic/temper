...
- The next likely work is executing the remaining non-label workflow effects (pull-request merge in Phase 9c, then idempotent PR create in Phase 10), addressing the limitations the robustness tests surfaced (compare-and-swap lease acquisition, applying reconciler actions automatically), or adding another concrete Forge backend (the `harness-forge-<provider>` convention and two reference backends are now in place).
