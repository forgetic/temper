...
- The next likely work is idempotent PR create (Phase 10, mirroring `Executor::ensure_issue`), addressing the limitations the robustness tests surfaced (compare-and-swap lease acquisition, applying reconciler actions automatically), or adding another concrete Forge backend (the `harness-forge-<provider>` convention and two reference backends are now in place).
