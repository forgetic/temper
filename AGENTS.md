...
- The next likely work is addressing the limitations the robustness tests surfaced (compare-and-swap lease acquisition, applying reconciler actions automatically), continuing the reference-workflow backlog (`dependency_gate`), or adding another concrete Forge backend (the `harness-forge-<provider>` convention and two reference backends are now in place).
