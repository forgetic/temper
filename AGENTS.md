...
- `harness-workflow` is the workflow/orchestration crate; it provides the typed workflow spec (`RawWorkflowSpec`), typed ids, validation diagnostics, and `ValidatedWorkflow` (Phase 2), plus artifact/Forge target mapping, workflow metadata blocks, and a classifier for Forge issues and pull requests (Phase 3), plus compilation of a validated workflow into role/prompt/tool/queue/label manifests and a runtime transition table (Phase 4). It depends on `harness-forge`. Runtime and recovery are still planned (see ADR 0007).
...
- The next likely work is continuing `harness-workflow` in phases (Phase 5: pure queue evaluation and transition planning), or adding another concrete Forge backend.
