...
- `harness-forge-forgejo` is the first real-world provider backend (the `harness-forge-<provider>` convention), adapting the `Forge` interface to Forgejo's HTTP API through a mockable `HttpClient` seam; it is offline-tested only (no live tests). Its pull-request surface is implemented: list/get/create/update pull requests, PR comments (Forgejo issue comments), requested-reviewer requests, native review events with one-call submission, and merge, with label/assignee updates reusing shared issue-endpoint helpers (`items.rs`) and best-effort optimistic concurrency via a per-artifact `ETag`/`updated_at` validator cache (`VersionCache`, `CasMode`). Native dependency links are implemented too (`dependencies.rs`): issue and pull-request `add/remove` plus `Issue::dependencies`/`PullRequest::dependencies` enrichment on reads, using Gitea's `/issues/{number}/dependencies` endpoint for both (PRs share issue numbers), with `404` tolerated as empty on reads but mapped to `InvalidRequest` on unsupported add/remove. CI is implemented too (`ci.rs`/`ci_match.rs`/`ci_time.rs`, with Actions DTOs in `types.rs`): Forgejo Actions runs/tasks are adapted to portable `CiJob`s via `list_ci_jobs(repo_id, query)`/`get_ci_job(id)` (the latter decodes its repo coordinate from the `CiJobId`, taking no repo parameter), matching runs by PR ref/head SHA/event payload (and PR head branch for pull-request events), grouping a run's tasks into attempts (latest wins), and mapping status/conclusion, with both endpoints' `workflow_runs`-wrapped arrays and string-or-integer timestamps tolerantly decoded and unavailable Actions endpoints (403/404) surfaced as `Backend` errors so the merge gate stays closed; CI log fetching stays outside the portable backend. Current user, repositories, repository labels, full issue CRUD, and the `Forge` trait wiring land in later phases. See `docs/reference/forgejo-backend.md`.
