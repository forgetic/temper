...
introduced="$(git -C "$repo_root" log --format=%H --diff-filter=A -1 -- scenarios/exact-head-feature-validation/scenario.toml)"
landing_base="$(git -C "$repo_root" rev-parse "$introduced^")"
