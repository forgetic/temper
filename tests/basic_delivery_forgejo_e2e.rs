...
//! `temper_testing::live_basic_delivery` harness.
//!
//! Run with:
...
use temper_testing::live_basic_delivery::{ScenarioBundle, TemperCommand, run_live_basic_delivery};
...
fn basic_delivery_run_sh_equivalent_converges() {
    ...
    let evidence = run_live_basic_delivery(scenario, temper).unwrap_or_else(|error| {
        panic!("{error}");
    });
