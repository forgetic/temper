...
- `harness-runner` contains backend-agnostic runner primitives. It provides read-only `scan`/`scan_role` functions that list Forge issues and pull requests, classify them, read public workflow `GateSignals`, apply queue activation, and emit one `WorkItem` per active queue member and subscriber role. It also defines `Agent`, `RoleTools`, `Worker`, and `RoleWorker`: agents service work items through role-scoped tools, and `RoleTools` is the only workflow-state mutation path exposed to agents (authorized transition execution plus the idempotent pull-request creation seam).
