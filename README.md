...
- The next likely implementation task is to execute the remaining non-label workflow effects (pull-request merge in Phase 9c, then idempotent PR create in Phase 10), address the limitations the robustness tests surfaced (compare-and-swap lease acquisition, applying reconciler actions automatically), or add another concrete Forge backend. See `docs/reference/robustness-guarantees.md`.
