...
- `harness-runner` contains backend-agnostic runner primitives. It provides read-only `scan`/`scan_role` functions that list Forge issues and pull requests, classify them, read public workflow `GateSignals`, apply queue activation, and emit one `WorkItem` per active queue member and subscriber role. It also defines `Agent`, `RoleTools`, `Worker`, `RoleWorker`, and `MechanicalWorker`: agents service work items through role-scoped tools, `RoleTools` is the only workflow-state mutation path exposed to agents (authorized transition execution plus the idempotent pull-request creation seam), and the mechanical worker runs reconcile → apply ticks for expired leases, partial transitions, dependency unblocks, and stale commands without spawning agents. `RunnerConfig`, `FixpointDriver`, `Scenario`/`Stage`, and `InProcessStage` now compose those workers into a process-layout-independent runnable world for layered tests and future production glue.
...
crates/
  ...
  harness-runner/           Backend-agnostic runner primitives: scanning, workers, drivers, stages.
docs/
