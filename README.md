...
- The reference-workflow backlog (Phases 9a–14) is complete, and lease acquisition is now a portable compare-and-swap: issues and pull requests carry an optimistic-concurrency `Version`, `UpdateIssue`/`UpdatePullRequest` take an `expected_version` precondition, and `LeaseManager` writes leases conditionally so two "no lease" acquirers cannot both win (ADR 0013). The next likely implementation task is automatically applying reconciler actions (including the dependency `Unblock`), or adding another concrete Forge backend. See `docs/reference/robustness-guarantees.md` and `docs/explanation/reference-workflow-roadmap.md`.
