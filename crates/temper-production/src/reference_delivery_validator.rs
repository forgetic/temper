...
async fn validate_children<F: Forge + ?Sized>(
    ...
    if landed == dependencies.len() && parent_blocked {
        ...
        report.diagnosis(
            "mechanical reconciliation should clear the dependency gate; inspect mechanical_reconciliation events",
        );
    }
    Ok(())
}
...
fn validate_child_metadata(
    ...
    if has_parent {
        ...
        report.missing(format!(
            "child {child_display}#{} carries parent back-reference to {source_repo}#{parent_number}",
            child.number
        ));
    }
    if metadata
