...
use harness_forge_forgejo::{ForgejoConfig, ForgejoForge, HttpMethod};
...
fn names(repos: &[harness_forge::Repository]) -> Vec<String> {
...
fn create_repository_conflict_maps_to_already_exists() {
    ...
    assert!(matches!(error, harness_forge::ForgeError::AlreadyExists(_)));
}
...
fn list_labels_maps_and_sorts_by_name() {
    ...
    assert_eq!(
        blocked.id,
        harness_forge::LabelId::new(format!("forgejo:{OWNER}/{REPO}:label:1"))
    );
...
fn upsert_label_creates_when_absent() {
    ...
    assert_eq!(
        label.id,
        harness_forge::LabelId::new(format!("forgejo:{OWNER}/{REPO}:label:7"))
    );
