...
use harness_forge::{CiJob, CiJobStatus, ForgeError, ForgeResult, PullRequestId, RepositoryId};
...
pub(crate) async fn read_ci_jobs<C: HttpClient>(
    ...
    for run in run_ids {
        ...
        for job in live_run_to_jobs(repo, repo_id, run, &live, target) {
            if job.conclusion == Some(harness_forge::CiJobConclusion::Cancelled) {
                continue;
            }
            jobs.push(job);
        }
...
mod tests {
    use super::*;
    use harness_forge::CiJobConclusion;
