//! Forgejo provisioning used by `harness-provision-forgejo`.

use std::collections::{BTreeMap, BTreeSet};
use std::fmt;
use std::path::Path;

use harness_forge::{CreateIssue, IssueQuery, ItemNumber, RepositoryId, RepositoryPath};
use harness_forge_forgejo::{ForgejoConfig, ForgejoForge};
use harness_runner::RoleBinding;
use harness_workflow::{ArtifactTarget, Effect, RoleId, ValidatedWorkflow};

use crate::forgejo_prep::commit_ci_sentinel;
use crate::forgejo_rest::{self, RestError, ROLE_PASSWORD};
...
pub enum ProvisionError {
    Rest(RestError),
    Forge(harness_forge::ForgeError),
...
impl From<harness_forge::ForgeError> for ProvisionError {
    fn from(err: harness_forge::ForgeError) -> Self {
        Self::Forge(err)
    }
}
...
pub fn format_secrets_env(provisioned: &Provisioned) -> String {
    ...
    out.push_str("# Generated by harness-provision-forgejo — live credentials, do not commit.\n");
    out.push_str(&format!(
...
mod tests {
    ...
    fn secrets_env_escapes_single_quotes() {
        ...
        assert!(env.contains("HARNESS_FORGEJO_TOKEN_CODE_REVIEWER='tok'"));
        assert!(env.contains(r"HARNESS_FORGEJO_PASSWORD_CODE_REVIEWER='pw-with-'\''-quote'"));
    }
}
