...
use harness_forge::{Forge, Issue, ItemNumber, RepositoryPath};
...
fn validate_serve_safety(args: &ProductChatServeArgs) -> Result<(), ProductChatError> {
    ...
    {
        return Err(ProductChatError::Runtime(
            "non-loopback bind requires HARNESS_PRODUCT_CHAT_SERVICE_TOKEN".into(),
        ));
    }
    Ok(())
}
...
fn run_http(
    ...
    eprintln!("harness-product-manager-chat: serving on {bind}");
    for stream in listener.incoming() {
        match stream {
            ...
            Err(error) => eprintln!("harness-product-manager-chat: accept failed: {error}"),
        }
    }
    Ok(())
}
