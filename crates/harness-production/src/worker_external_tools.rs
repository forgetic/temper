...
use harness_runner::{ExternalToolExecutors, RunnerConfig, CODING_WORKSPACE_TOOL_ID};
use harness_workflow::{CompiledWorkflow, ExternalToolId};

use crate::coding_workspace::LocalGitCodingWorkspace;
...
pub(crate) fn configure_external_tool_executors(
    ...
    let workspace = Arc::new(workspace) as Arc<dyn harness_runner::CodingWorkspace>;
    ...
    if bound == 0 {
        eprintln!(
            "harness-worker: coding workspace env is configured, but no compiled role declares {CODING_WORKSPACE_TOOL_ID}"
        );
    }
    Ok(executors)
}
