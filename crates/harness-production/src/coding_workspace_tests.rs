...
use harness_workflow::{ArtifactKindId, ArtifactSource, ExternalToolId, QueueId, RoleId};
use std::fs;
...
impl TestRepo {
    fn new(name: &str) -> Self {
        ...
        git(&path, &["config", "user.name", "Harness Test"]).expect("git user name configured");
...
fn request() -> CodingWorkspaceRequest {
    CodingWorkspaceRequest {
        repository: harness_runner::CodingWorkspaceRepository {
            id: RepositoryId::new("repo-1"),
            owner: "acme".to_string(),
            name: "service".to_string(),
            default_branch: "main".to_string(),
        },
        work_item: harness_runner::CodingWorkspaceWorkItem {
            role: RoleId::new("engineer"),
            queue: QueueId::new("code_ready"),
            kind: ArtifactKindId::new("code"),
            target: ArtifactSource::Issue {
                number: harness_forge::ItemNumber::new(7),
            },
        ...
        guidance: harness_runner::CodingWorkspaceGuidance {
            ...
            tool_constraints: vec!["No .harness-only diffs.".to_string()],
        },
    }
}
...
fn local_git_workspace_rejects_synthetic_only_diff() {
    ...
    let workspace = local_workspace(
        &repo.path,
        "mkdir -p .harness-ci && printf 'marker\n' > .harness-ci/ok.txt",
    );
    ...
    assert!(error.contains(".harness-ci/ok.txt"));
}
...
fn coding_workspace_tool_id_constant_matches_workflow_convention() {
    assert_eq!(
        harness_runner::CODING_WORKSPACE_TOOL_ID,
