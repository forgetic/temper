...
//! `Runtime::current_handle` (skein removed it); production threads a
//! `RuntimeHandle` explicitly and the lab provides a `Spawner` seam. That
//! explicit-capability design is exactly what lets the same shells run under
//! both runtimes.
