...
//!   cargo test -p temper-agents --test chatgpt_oauth_live -- --ignored --nocapture
//! ```
...
use temper_agents::decision::run_decision;
use temper_agents::{
...
impl TempAuthFile {
    fn seed(contents: &str) -> Self {
        let path =
            std::env::temp_dir().join(format!("temper-agents-a3-auth-{}.json", std::process::id()));
