...
use temper_forgejo_ops::forgejo_rest::{self, RestError};
...
fn guard_comment_body(role: &GuardRole, files: &[String]) -> String {
    ...
    let mut body = format!(
        concat!(
            "{}\n",
            "Temper dogfood safety guard is {}: this PR has no meaningful ",
            "product diff after excluding internal Temper bookkeeping paths."
        ),
...
mod tests {
    ...
    fn classifies_product_file_as_meaningful() {
        assert_eq!(
            safety_for_files(vec![
                "crates/temper-interaction-service/src/interaction_service.rs".into()
            ]),
            DiffSafety::Meaningful {
                files: vec!["crates/temper-interaction-service/src/interaction_service.rs".into()]
            }
        );
    }
