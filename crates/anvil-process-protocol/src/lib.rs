...
//!
//! This crate is the **contract** between an orchestration worker
//! (`smith-worker`) and an out-of-process **anvil agent**. It is serde-only and
//! depends on nothing else, so a third-party agent can speak it without pulling
//! in anvil's pi-SDK internals, and the worker can speak it without pulling in
//! anvil at all beyond this crate.
//!
//! # Shape
//!
//! The exchange is deliberately narrow (see `smith/agent-process-split.md`):
//!
