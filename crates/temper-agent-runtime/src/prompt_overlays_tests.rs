...
#[test]
fn resolve_config_dir_reads_anvil_config_dir_env() {
    // With no explicit override, ANVIL_CONFIG_DIR is used verbatim (no /anvil
    // suffix — the env names the dir directly).
    let resolved = resolve_config_dir_from(
        None,
        Some(PathBuf::from("/env/anvil")),
        Some(PathBuf::from("/xdg")),
        Some(PathBuf::from("/home/tester")),
    )
    .expect("env resolves");
    assert_eq!(resolved, PathBuf::from("/env/anvil"));
}

#[test]
fn resolve_config_dir_falls_back_to_xdg_then_home() {
    // No explicit override, no ANVIL_CONFIG_DIR: $XDG_CONFIG_HOME/anvil wins.
    let resolved = resolve_config_dir_from(
        None,
        None,
    ...
    .expect("xdg resolves");
    assert_eq!(resolved, PathBuf::from("/xdg/config/anvil"));

    // With XDG also unset, fall back to ~/.config/anvil.
    let resolved = resolve_config_dir_from(None, None, None, Some(PathBuf::from("/home/tester")))
        .expect("home resolves");
    assert_eq!(resolved, PathBuf::from("/home/tester/.config/anvil"));
