...
use temper_protocol_activity::{
    ...
    AgentScopeV1, CaptureModeV1, DroppedEventKindV1, FailureCodeV1, FailureInfoV1, RunFailedV1,
    RunFinishedV1, RunStartedV1, RunStatusV1, StopReasonV1, TraceGapV1, UsageV1,
};
...
fn event_pages_follow_appends_and_jsonl_export_preserves_canonical_order() {
    ...
    temper_engine_io::block_on_with(move |_cx, handle| async move {
        ...
        let sequences = exported
            .lines()
            .map(|line| {
                serde_json::from_str::<AgentRunEventV1>(line)
                    .expect("event JSON")
                    .seq
            })
            .collect::<Vec<_>>();
