...
mod tests {
    ...
    #[test]
    fn inserts_ledger_before_workflow_metadata() {
        let metadata = render_metadata_block(&WorkflowMetadata::default());
        let body = format!("Human text.\n\n{metadata}");
        let block = format!("{}\nledger\n{}", ledger_marker("pr-for-code-7"), LEDGER_END);

        let updated = insert_run_ledger_block(&body, &block).expect("merge succeeds");

        assert!(updated.starts_with("Human text.\n\n<!-- temper-run-ledger"));
        assert!(updated.contains("<!-- /temper-run-ledger -->\n\n<!-- temper:workflow"));
        assert!(updated.ends_with(&metadata));
    }

    #[test]
    fn repeated_progress_replaces_same_ledger() {
        let first = format!("{}\none\n{}", ledger_marker("key"), LEDGER_END);
        let second = format!("{}\ntwo\n{}", ledger_marker("key"), LEDGER_END);
        let body = upsert_run_ledger_block("Issue", "key", &first, true)
            .expect("first merge succeeds")
            .expect("body changes");

        let updated = upsert_run_ledger_block(&body, "key", &second, true)
    ...
    #[test]
    fn progress_does_not_replace_finalized_ledger() {
        let finalized = format!(
            "{}\n- Current status: continued in PR #2\n{}",
            ledger_marker("key"),
            LEDGER_END
        );
        let progress = format!("{}\ncheckpointed\n{}", ledger_marker("key"), LEDGER_END);
