...
use crate::ids::{parse_repository_id, user_login};
...
impl<C: HttpClient> GitHubForge<C> {
    ...
    /// Reads a branch through GitHub's native branch endpoint.
    ...
    ///
    /// GitHub's listing order is not contractual, so the requested sort is
    ...
    /// own handle, else `POST /orgs/{owner}/repos`. GitHub reports an existing
    /// name as `422` with an "already exists" message, which maps to
    /// [`ForgeError::AlreadyExists`]; other `422`s stay
    ...
    ///
    /// GitHub ignores a `default_branch` field on create (the setting can only
    /// be patched once the branch exists), so a requested default branch that
    /// differs from the provider's is applied with a follow-up
    /// `PATCH /repos/{owner}/{repo}` once the repository exists.
    pub async fn create_repository(&self, input: CreateRepository) -> ForgeResult<Repository> {
        ...
        if !response.is_success() {
            return Err(match response.status {
                // GitHub signals an existing repository name with a 422 whose
                // message mentions the name already existing.
                422 if response.body.contains("already exists") => {
        ...
        if input.default_branch.is_empty() || created.default_branch == input.default_branch {
            return Ok(created);
        }
        // Align the default branch with the request. This requires the branch
        // to exist; on a brand-new empty repository GitHub rejects it, which is
        // surfaced rather than silently ignored.
        let patch = serde_json::json!({ "default_branch": input.default_branch }).to_string();
        let api_path = format!("/repos/{}/{}", input.owner, input.name);
    ...
    ///
    /// GitHub addresses labels by name, so the existing label is probed with
    /// `GET …/labels/{name}` and patched in place (`PATCH …/labels/{name}`)
    /// when present; a missing name is created (`POST …/labels`).
    ...
    pub async fn upsert_label(
        ...
        let named = format!("{base}/{}", input.name);
        let exists = self
            .request_optional("get label", HttpMethod::Get, &named, Vec::new(), None)
            .await?
            .is_some();
