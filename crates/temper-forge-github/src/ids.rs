...
//!
//! These encodings are private to the GitHub backend. Workflow code must treat
//! the resulting ids as opaque and never parse them; only this module maps
//! between portable [`temper_forge_model`] id newtypes and GitHub coordinates.
//!
//! Shapes:
//!
...
//! - review: `github:{owner}/{repo}:review:{id}`
//! - label: `github:{owner}/{repo}:label:{id}`
//! - CI job: `github:{owner}/{repo}:job:{job_id}` (an Actions workflow job;
//!   GitHub job ids are repository-stable, so no run coordinate is needed)
//! - user: the GitHub login, unprefixed
//!
//! Owner and repository names cannot contain `:` or `/` on GitHub, so splitting
...
const SCHEME: &str = "github";

/// Owner/name coordinate for a GitHub repository.
...
/// Decoded coordinate for a GitHub Actions CI job.
#[derive(Clone, Debug, Eq, PartialEq)]
...
fn invalid_id(kind: &str, raw: &str) -> ForgeError {
    ForgeError::InvalidRequest(format!("not a github {kind} id: {raw:?}"))
}
...
/// Parses a `github:{owner}/{repo}:{tag}:{number}` id.
...
pub(crate) fn parse_ci_job_id(id: &CiJobId) -> ForgeResult<CiJobCoord> {
    let (repo, job_id) = parse_numbered("ci job", "job", id.as_str())?;
    Ok(CiJobCoord { repo, job_id })
}

// --- user --------------------------------------------------------------------

/// Encodes a GitHub login as a portable [`UserId`].
pub(crate) fn format_user_id(login: &str) -> UserId {
    UserId::new(login)
}

/// Decodes a [`UserId`] back into a GitHub login, rejecting empty ids.
...
mod tests {
    ...
    fn repository_id_round_trips() {
        ...
        assert_eq!(id.as_str(), "github:acme/widgets");
    ...
    fn issue_and_pull_ids_round_trip() {
        ...
        assert_eq!(issue.as_str(), "github:acme/widgets:issue:7");
        ...
        assert_eq!(pull.as_str(), "github:acme/widgets:pull:42");
    ...
    fn ci_job_id_round_trips() {
        ...
        assert_eq!(id.as_str(), "github:acme/widgets:job:987654");
    ...
    fn rejects_foreign_shapes() {
        ...
        assert!(parse_repository_id(&RepositoryId::new("github:noslash")).is_err());
        assert!(parse_repository_id(&RepositoryId::new("forgejo:acme/widgets")).is_err());
        assert!(parse_issue_id(&IssueId::new("github:acme/widgets:pull:7")).is_err());
        assert!(parse_issue_id(&IssueId::new("github:acme/widgets:issue:abc")).is_err());
        assert!(parse_pull_request_id(&PullRequestId::new("github:acme/widgets:issue:7")).is_err());
        assert!(parse_label_id(&LabelId::new("label-acme-widgets")).is_err());
        assert!(parse_ci_job_id(&CiJobId::new("github:acme/widgets:actions:1:2:3")).is_err());
        assert!(parse_repository_id(&RepositoryId::new("github:/widgets")).is_err());
    }
}
