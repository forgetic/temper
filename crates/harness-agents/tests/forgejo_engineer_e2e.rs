...
//!   cargo test -p harness-agents --test forgejo_engineer_e2e -- --ignored
//! ```
//!
...
use harness_testing::agents::FakeArchitect;
use harness_testing::forgejo_server::{
...
fn enabled() -> bool {
    ...
    let agents = std::env::var("HARNESS_FORGEJO_AGENTS").ok().as_deref() == Some("1");
    if e2e && agents {
        return true;
    }
    eprintln!(
...
/// openable and passes CI — exactly the side effects the harness's
