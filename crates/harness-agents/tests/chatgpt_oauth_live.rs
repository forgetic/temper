...
//!
//! `#[ignore]`d **and** gated on `HARNESS_CHATGPT_OAUTH=1`: the default
//! `cargo test` never makes this call.
//!
//! ```sh
//! HARNESS_CHATGPT_OAUTH=1 \
//!   cargo test -p harness-agents --test chatgpt_oauth_live -- --ignored --nocapture
//! ```
...
#[ignore = "makes real ChatGPT (OpenAI Codex) OAuth calls; \
            run with HARNESS_CHATGPT_OAUTH=1 -- --ignored --nocapture"]
fn chatgpt_oauth_validation() {
    if std::env::var("HARNESS_CHATGPT_OAUTH").ok().as_deref() != Some("1") {
        eprintln!(
            "skipping ChatGPT OAuth live validation: set HARNESS_CHATGPT_OAUTH=1 (reads the real \
