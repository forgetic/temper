...
pub const AUTH_FILE_ENV: &str = "HARNESS_AGENTS_AUTH_FILE";
/// Env var overriding the codex model id.
pub const CODEX_MODEL_ENV: &str = "HARNESS_AGENTS_CODEX_MODEL";
/// Env var overriding the codex OAuth token endpoint (used by tests).
const TOKEN_URL_ENV: &str = "HARNESS_AGENTS_CODEX_TOKEN_URL";
...
mod tests {
    ...
    fn write_fixture(contents: &str) -> Fixture {
        ...
        let path = std::env::temp_dir().join(format!(
            "harness-agents-oauth-test-{}-{id}.json",
            std::process::id()
        ));
