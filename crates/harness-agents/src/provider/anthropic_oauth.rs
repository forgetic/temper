...
//!
//! This is intentionally Harness-local: the SDK provider remains unpatched. The
...
pub const ANTHROPIC_MODEL_ENV: &str = "HARNESS_AGENTS_ANTHROPIC_MODEL";
/// Env var overriding the Anthropic OAuth token endpoint (used by tests).
const TOKEN_URL_ENV: &str = "HARNESS_AGENTS_ANTHROPIC_TOKEN_URL";
...
mod tests {
    ...
    fn write_fixture(contents: &str) -> Fixture {
        ...
        let path = std::env::temp_dir().join(format!(
            "harness-agents-anthropic-oauth-test-{}-{id}.json",
            std::process::id()
        ));
