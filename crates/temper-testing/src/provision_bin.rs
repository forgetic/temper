//! Library backing the `temper-testing-provision` operator binary.
//!
...
pub const USAGE: &str = concat!(
    "temper-testing-provision --base-url <url> --owner <org> --name <repo> --out <path>\n",
    "  the admin token comes from the environment, never argv: ",
    "TEMPER_FORGEJO_ADMIN_TOKEN (required)",
);
...
pub fn format_secrets_env(provisioned: &Provisioned) -> String {
    ...
    out.push_str("# Generated by temper-testing-provision — live credentials, do not commit.\n");
    out.push_str(&format!(
...
mod tests {
    ...
    fn secrets_env_is_sourceable_and_role_keyed() {
        ...
        assert!(env.contains("TEMPER_FORGEJO_OWNER='acme'\n"));
        assert!(env.contains("TEMPER_FORGEJO_REPO='service'\n"));
        assert!(env.contains("TEMPER_FORGEJO_USER_ARCHITECT='architect'\n"));
        assert!(env.contains("TEMPER_FORGEJO_TOKEN_ARCHITECT='tok-architect'\n"));
        assert!(env.contains("TEMPER_FORGEJO_TOKEN_ENGINEER='tok-engineer'\n"));
