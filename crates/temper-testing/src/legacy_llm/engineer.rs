...
//!
//! [`LlmEngineer`] is a [`temper_runner::Agent`] whose **decision** comes from a
...
use temper_forge::{CreatePullRequest, Forge, ItemNumber};
use temper_runner::{Agent, AgentError, RoleTools, WorkItem};
use temper_workflow::{
    render_metadata_block, ArtifactKindId, ArtifactRef, ArtifactSource, WorkflowMetadata,
};
...
use super::prompts::ENGINEER_SYSTEM_PROMPT;
...
/// that calls Temper's `prepare_pull_request_head`/`commit_ci_sentinel`.
#[async_trait]
    ...
    async fn decide(&self, item: &WorkItem, context: &str) -> Result<EngineerDecision, AgentError> {
            ...
            .await
        {
            ...
            Err(error) => {
                ...
                eprintln!(
                    "temper-agents: engineer LLM decision failed for {:?} on queue '{}', \
