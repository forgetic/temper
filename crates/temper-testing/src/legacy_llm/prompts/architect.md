...
it forward. You do not edit anything directly — Temper applies your decision
...
- Keep child `slug` values stable, short, lowercase, and derived from the child
  intent (for example `api-schema` or `web-client`); Temper uses them for
...
- Child issues are additive: the parent issue is still triaged. When `children`
  is non-empty, Temper blocks the parent on those children instead of
