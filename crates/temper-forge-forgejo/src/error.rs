...
pub(crate) fn map_transport_error(error: HttpError) -> ForgeError {
    ForgeError::Backend(format!("forgejo request failed: {error}"))
}

/// Classifies a non-success HTTP response into a portable [`ForgeError`].
///
/// - `404` → [`ForgeError::NotFound`] (lookups that tolerate absence should
///   convert `404` to `Ok(None)` before calling this).
/// - `409`/`412` → [`ForgeError::Conflict`] (already-exists / failed
///   precondition such as a stale conditional write).
/// - `400`/`422` → [`ForgeError::InvalidRequest`].
/// - `401`/`403`/`5xx`/anything else → [`ForgeError::Backend`].
...
mod tests {
    ...
    fn maps_status_codes_to_categories() {
        ...
        assert!(matches!(
            map_status_error("auth", &response(403, "forbidden")),
            ForgeError::Backend(_)
        ));
        assert!(matches!(
