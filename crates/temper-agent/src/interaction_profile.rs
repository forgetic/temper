...
//!
//! This module uses a anvil-owned interactive profile config to run Temper's
//! generic `ConversationRequest`/`ConversationReply` process contract through
//! anvil's existing provider/decision core. The config is deliberately provider-
...
/// Fixed anvil protocol instruction for provider calls that must return Temper's
...
impl GenericInteractionResponder {
    /// Builds a generic responder that calls anvil's provider decision core.
