...
pub use super::anthropic_model::{
    ANTHROPIC_MODEL_ENV, CLAUDE_CODE_SYSTEM_IDENTITY, DEFAULT_ANTHROPIC_MODEL, context_window_for,
    max_output_tokens_for, request_headers, resolve_anthropic_model,
    resolve_anthropic_subagent_model,
};

/// Env var overriding the Anthropic OAuth token endpoint (used by tests). The
/// name lives here so the agent's `entry` (the sole env reader) and the worker's
/// env-injection map agree; nothing in this crate reads it.
pub const ANTHROPIC_TOKEN_URL_ENV: &str = "TEMPER_AGENTS_ANTHROPIC_TOKEN_URL";
...
/// Where the Anthropic OAuth credential is read from, plus the token endpoint to
/// refresh against.
#[derive(Clone)]
pub struct AnthropicOAuthSettings {
    auth_file: PathBuf,
    /// The refresh token endpoint; `None` uses the compiled-in
    /// [`ANTHROPIC_TOKEN_URL`] (tests override it via [`ANTHROPIC_TOKEN_URL_ENV`],
    /// resolved in the agent's `entry`).
...
impl AnthropicOAuthSettings {
    ...
    /// `TEMPER_AGENTS_AUTH_FILE` value already folded in by the host), else the
    /// SDK default (`~/.pi/agent/auth.json`). The `token_url` override (the
    /// [`ANTHROPIC_TOKEN_URL_ENV`] value the host read) redirects token refresh;
    /// `None` keeps the compiled-in endpoint. Reads no environment.
...
impl AnthropicEntry {
    ...
    /// refresh token, updating the in-memory entry in place. `token_url` overrides
    /// the endpoint when supplied (a test-only redirect resolved in the agent's
    /// `entry`); otherwise the compiled-in [`ANTHROPIC_TOKEN_URL`] is used.
    async fn refresh(&mut self, token_url: Option<&str>) -> Result<(), ProviderError> {
        let token_url = token_url.unwrap_or(ANTHROPIC_TOKEN_URL);
