//! A throwaway, host-mode `forgejo-runner` for the env-gated CI harness.
//!
...
//! only an `#[ignore]`d, `HARNESS_FORGEJO_E2E=1`-gated test constructs one. CI
...
impl ForgejoRunner {
    ...
    pub fn register(server: &ForgejoServer) -> Result<Self, RunnerError> {
        ...
        let name = format!("harness-runner-{}-{id}", std::process::id());
