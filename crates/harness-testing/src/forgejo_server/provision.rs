//! Provisioning + identity for the env-gated Forgejo end-to-end harness.
//!
...
//!
//! Everything here is test-harness state and lives entirely outside
//! `harness-forge` / `RunnerConfig`: the role map is keyed by [`RoleId`] but is
...
/// Errors raised while provisioning a Forgejo for the e2e harness.
#[derive(Debug)]
...
async fn upsert_labels(
    ...
    use harness_forge::{RepositoryPath, UpsertLabel};
