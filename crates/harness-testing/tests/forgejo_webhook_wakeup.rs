...
use harness_forge::{CiJobQuery, Forge, PullRequestQuery};
...
fn enabled() -> bool {
    if std::env::var("HARNESS_FORGEJO_E2E").ok().as_deref() == Some("1") {
        ...
        eprintln!(
            "skipping Forgejo webhook wakeup e2e: set HARNESS_FORGEJO_E2E=1 to enable \
...
fn spawn_worker(
    ...
    command
        ...
        .arg(repo)
        .arg("--root")
        .arg(std::env::temp_dir().join("harness-forgejo-webhook-unused"))
        .arg("--clock")
        .arg("wall")
        .arg("--poll-ms")
