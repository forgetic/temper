...
//!   cargo test -p harness-testing --test forgejo_worker -- --ignored
//! ```
//!
...
//!   (`HARNESS_FORGEJO_TOKEN`, plus optional `HARNESS_FORGEJO_USERNAME/PASSWORD`);
...
use harness_testing::worker_bin::{
...
fn enabled() -> bool {
    ...
    eprintln!(
        "skipping Forgejo worker e2e test: set HARNESS_FORGEJO_E2E=1 to enable \
...
async fn forgejo_role_worker_ticks_against_a_running_server() {
    ...
    let argv = vec![
        ...
        "--root".to_string(),
        std::env::temp_dir()
            .join("harness-forgejo-worker-unused")
            .to_string_lossy()
            .into_owned(),
        "--clock".to_string(),
        "wall".to_string(),
