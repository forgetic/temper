...
use harness_forge::{BranchRef, CreateIssue, CreatePullRequest};
use harness_forge_forgejo::{ForgejoConfig, ForgejoForge};
use harness_production::coding_workspace::LocalGitCodingWorkspace;
use harness_production::forgejo_rest;
use harness_production::pr_diff_guard::{safety_for_files, DiffSafety};
use harness_runner::{
...
use harness_testing::forgejo_server::{provision, ForgejoServer};
...
fn enabled() -> bool {
    let enabled = std::env::var("HARNESS_FORGEJO_E2E").ok().as_deref() == Some("1");
    if !enabled {
        eprintln!(
            "skipping Forgejo workspace PR proof: set HARNESS_FORGEJO_E2E=1 \
...
#[test]
#[ignore = "boots a real Forgejo; run with HARNESS_FORGEJO_E2E=1 -- --ignored"]
fn local_git_workspace_pushes_meaningful_pr_diff() {
    ...
    let temp = TempDir::new("harness-forgejo-workspace-pr");
    ...
    git(&checkout, &["config", "user.name", "Harness Engineer"]);
