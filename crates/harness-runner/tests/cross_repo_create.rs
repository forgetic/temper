use harness_forge::{CreateIssue, CreateRepository, Forge, IssueQuery, ItemNumber, RepositoryId};
use harness_forge_filesystem::FilesystemForge;
use harness_forge_memory::MemoryForge;
use harness_runner::RoleTools;
use harness_testing::{block_on, workflow};
...
impl TempRoot {
    fn new() -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "harness-runner-cross-repo-create-{}-{id}",
            std::process::id()
        ));
...
fn memory_existing_correlated_issue_is_found_and_gets_parent_backref() {
    ...
    let existing = block_on(forge.create_issue(
        ...
        CreateIssue {
            ...
            body: format!(
                "body\n\n{}",
                harness_workflow::render_metadata_block(&harness_workflow::WorkflowMetadata {
                    correlation_key: Some(key.clone()),
                    ..harness_workflow::WorkflowMetadata::default()
                })
            ),
            labels: Vec::new(),
            assignees: Vec::new(),
