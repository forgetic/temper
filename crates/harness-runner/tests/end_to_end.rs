...
use harness_workflow::{InMemoryJournal, LeasePolicy};
...
use harness_testing::ci::{FailThenPassCiPolicy, FilesystemCiSink, FixedCiPolicy, MemoryCiSink};
...
fn role_forges<F, M>(
    config: &harness_runner::RunnerConfig,
    mut map: M,
) -> Vec<(harness_workflow::RoleId, F)>
where
    M: FnMut(harness_forge::User) -> F,
{
    config
        .role_bindings
        .iter()
...
async fn run_cross_repo_fleet<F, S>(
    forge: &F,
    role_forges: &[(harness_workflow::RoleId, F)],
    repos: Vec<RepositoryTarget>,
    ci_sink: S,
    budget: u64,
) -> Result<harness_runner::RunReport, Box<dyn std::error::Error>>
where
    F: Forge + 'static,
    S: CiSink + Send + Sync,
{
