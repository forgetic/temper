...
use harness_testing::{block_on, repo_input, runner_config, user, workflow};
use harness_workflow::{ArtifactKindId, QueueId, RoleId, TransitionId};
...
impl TempRoot {
    fn new(suite: &str) -> Self {
        let path = std::env::temp_dir().join(format!(
            "harness-runner-filesystem-hints-{suite}-{}",
            std::process::id()
        ));
...
fn pull_request_review_hint_wakes_broad_mechanical_route() {
    ...
    let pr = block_on(writer.create_pull_request(
        &repo,
        harness_testing::pull_request_input(
            &repo,
            "implementation",
            "",
            "feature",
